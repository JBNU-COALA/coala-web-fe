...
(async () => {
  ...
  await page.route('**/api/**', async route => {
    ...
    else if (path === '/api/users/999') return route.fulfill({ status: 404, json: { message: 'Not found' } });
    else if (path === '/api/info' && method === 'POST') {
      const payload = route.request().postDataJSON();
      assert.equal(payload.boardId, undefined);
      assert.equal(payload.filter, 'resource');
      infoArticle = { ...payload, id: 876, source: '운영자', viewCount: 0, bookmarkCount: 0, authorId: 1 };
      data = infoArticle;
    }
    else if (path === '/api/info/876') data = infoArticle;
  ...
  await page.getByRole('link', { name: '출석 체크', exact: true }).first().click();
  await page.getByLabel('연결할 조 (선택)').selectOption('42');
  ...
  await page.goto('http://127.0.0.1:3000/users/999');
  await page.getByText('프로필을 불러오지 못했습니다.', { exact: true }).waitFor();
  await page.goto('http://127.0.0.1:3000/community/info/posts/new');
  await page.getByLabel('정보공유 분류', { exact: true }).selectOption('resource');
  await page.getByLabel('게시글 제목', { exact: true }).fill('API 정보공유 테스트');
  await page.getByLabel('게시글 본문 입력', { exact: true }).fill('실제 API 응답만 사용합니다.');
  await page.getByRole('button', { name: '발행하기', exact: true }).click();
  await page.waitForURL('**/community/info/posts/876');
  await page.getByRole('heading', { name: 'API 정보공유 테스트' }).waitFor();
  await page.goto('http://127.0.0.1:3000/community/info/11/posts/876');
  await page.waitForURL('**/community/info/posts/876');
  console.log('PASS: missing profile, info creation without fabricated board ID, legacy redirect');
  await browser.close();

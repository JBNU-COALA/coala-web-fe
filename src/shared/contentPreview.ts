import { normalizeMarkdownAttachmentUrl } from './markdown'
...
export function extractFirstContentImage(content: string) {
  ...
  if (htmlImage) return normalizeMarkdownAttachmentUrl(stripWrappingUrlBrackets(htmlImage))

  const markdownImage = content.match(markdownImagePattern)?.[1]
  if (markdownImage) return normalizeMarkdownAttachmentUrl(stripWrappingUrlBrackets(markdownImage))

  const bareImageUrl = content.match(bareImageUrlPattern)?.[0] ?? ''
  return bareImageUrl ? normalizeMarkdownAttachmentUrl(bareImageUrl) : ''
}
